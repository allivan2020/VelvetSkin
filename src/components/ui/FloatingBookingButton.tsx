...
const FloatingBookingButton = () => {
  // Нам больше не нужен mounted, если мы используем { ssr: false } в layout.tsx
  ...
  return (
    ...
    >
      <Link
        href="#booking-modal"
        className="group relative flex items-center justify-center px-8 py-4 rounded-full bg-white/40 backdrop-blur-xl border border-white/60 shadow-[0_10px_30px_-10px_rgba(0,0,0,0.3),0_0_20px_rgba(252,178,94,0.3)] hover:shadow-[0_10px_40px_-10px_rgba(0,0,0,0.4),0_0_30px_rgba(252,178,94,0.5)] transition-all duration-500 active:scale-95 no-underline"
      >
        ...
        <span className="relative flex h-2 w-2 ml-2">
          <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#c49f2d] opacity-75"></span>
          <span className="relative inline-flex rounded-full h-2 w-2 bg-[#c49f2d]"></span>
        </span>
      </Link>
    </motion.div>
  );
